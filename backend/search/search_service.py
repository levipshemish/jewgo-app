...
from .core.search_types import SearchType, SearchResult, SearchFilters, SearchResponse, SearchMetadata
from .core.search_config import get_search_config, SearchConfig
...
class SearchService:
        ...
        try:
            ...
            if self.config.postgresql.enabled:
                self.providers['postgresql'] = PostgreSQLSearchProvider(
                    self.db_session,
                    config=self.config.postgresql.__dict__
                )
    ...
    async def search(
        ...
        **kwargs
        ...
        try:
            ...
            if not provider:
                raise SearchError(f"Search provider '{search_type}' not available", search_type, query)
            
            # Check provider health
            if not await provider.is_healthy():
                raise SearchError(f"Search provider '{search_type}' is unhealthy", search_type, query)
    ...
    async def get_suggestions(self, query: str, search_type: Optional[str] = None, limit: int = 10) -> List[str]:
    ...
    async def get_available_providers(self) -> Dict[str, Dict[str, Any]]:
        ...
        for provider_name, provider in self.providers.items():
            try:
                ...
                logger.warning(f"Failed to get info for provider {provider_name}", error=str(e))
                provider_info[provider_name] = {
                    'type': 'unknown',
                    'name': provider_name,
                    'healthy': False,
