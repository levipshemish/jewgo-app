...
class SearchConfig:
    ...
    def from_env(cls) -> 'SearchConfig':
        ...
        config.postgresql.enabled = os.getenv('POSTGRESQL_SEARCH_ENABLED', 'true').lower() == 'true'
        config.postgresql.fuzzy_threshold = float(os.getenv('POSTGRESQL_FUZZY_THRESHOLD', '0.3'))
        config.postgresql.max_results = int(os.getenv('POSTGRESQL_MAX_RESULTS', '100'))
        
        # Vector, semantic, and hybrid search configurations removed
        
        # Cache configuration
        config.cache.enabled = os.getenv('SEARCH_CACHE_ENABLED', 'true').lower() == 'true'
        config.cache.redis_url = os.getenv('REDIS_URL')
        config.cache.default_ttl = int(os.getenv('SEARCH_CACHE_TTL', '300'))
        
        # Performance configuration
        config.performance.timeout_seconds = int(os.getenv('SEARCH_TIMEOUT_SECONDS', '10'))
        config.performance.max_results_per_request = int(os.getenv('SEARCH_MAX_RESULTS', '100'))
        
        # General settings
        config.default_search_type = os.getenv('DEFAULT_SEARCH_TYPE', 'postgresql')
        config.enable_logging = os.getenv('SEARCH_LOGGING_ENABLED', 'true').lower() == 'true'
        config.enable_metrics = os.getenv('SEARCH_METRICS_ENABLED', 'true').lower() == 'true'
