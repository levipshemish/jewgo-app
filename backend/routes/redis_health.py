    ...
    try:
        ...
        if hasattr(current_app, 'cache'):
            try:
                ...
                cache_stats = {
                    'cache_type': getattr(current_app.cache, 'config', {}).get('CACHE_TYPE', 'unknown'),
                    'cache_timeout': getattr(current_app.cache, 'config', {}).get('CACHE_DEFAULT_TIMEOUT', 0)
                }
            except Exception:
...
def redis_test():
    ...
    try:
        ...
        test_results['hash_get_all'] = r.hgetall(test_hash) == {b"field1": b"value1", b"field2": b"value2"}
        r.delete(test_hash)
...
def cache_health_check():
    ...
    try:
        ...
        cache_metrics = {
            'cache_operations': {
                'total_operations': health_status.get('error_count', 0) + 100,  # Estimate
                'error_rate': health_status.get('error_count', 0) / max(health_status.get('error_count', 0) + 100, 1) * 100,
        ...
        if health_status.get('error_count', 0) > 10:
            cache_metrics['recommendations'].append('High error rate detected - check Redis connectivity')
        
        if health_status.get('cache_type') == 'memory' and health_status.get('redis_connected') is False:
            cache_metrics['recommendations'].append('Using memory cache - Redis connection failed')
        
        if health_status.get('memory_cache_size', 0) > 1000:
            cache_metrics['recommendations'].append('Large memory cache - consider Redis for better performance')
