...
# Create blueprint
shtetl_store_bp = Blueprint('shtetl_store', __name__, url_prefix='/api/v4/shtetl/stores')
...
def create_store():
    ...
    try:
        ...
        logger.info(f"Store created: {result.get('store_id')} by user {current_user.get('id')}")
...
def get_stores():
    ...
    try:
        ...
        return success_response("Stores retrieved successfully", {
            'stores': stores_list,
...
def search_stores():
    ...
    try:
        ...
        return success_response("Stores search completed", {
            'stores': stores_list,
            'total': len(stores_list),
...
def get_store_analytics(store_id):
    ...
    try:
        ...
        analytics_dict = {
            ...
            'start_date': analytics.start_date.isoformat() if analytics.start_date else None,
...
def get_store_messages(store_id):
    ...
    try:
        ...
        return success_response("Messages retrieved successfully", {
            'messages': messages,
            'total': len(messages),
            'limit': limit,
            'offset': offset
...
def get_plan_limits():
    ...
    try:
        ...
        return success_response("Plan limits retrieved successfully", {
            'plan_type': plan_type,
            'limits': plan_limits
        })
...
def admin_suspend_store(store_id):
    ...
    try:
        ...
        logger.info(f"Store suspended: {store_id} by admin {current_user.get('id')} - Reason: {reason}")
        return success_response(message)
