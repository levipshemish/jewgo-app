...
class V4MonitoringSystem:
    ...
    def record_request_metric(self, endpoint: str, method: str, response_time_ms: float, 
    ...
    def record_error_metric(self, endpoint: str, error_type: str, error_message: str, 
    ...
    def _check_error_rate(self, endpoint: str):
        ...
        recent_errors = [e for e in self.metrics['errors'][endpoint] 
                        if datetime.fromisoformat(e['timestamp']) > datetime.now() - timedelta(minutes=5)]
        
        recent_requests = [r for r in self.metrics['requests'][endpoint] 
    ...
    def _check_alerts(self):
        ...
        recent_failures = [f for f in self.metrics['database']['connection_failures'] 
        ...
        if len(recent_failures) >= self.thresholds['database_connection_failures']:
            self._create_alert('database_connection_issues', {
        ...
        for category in self.metrics:
            if isinstance(self.metrics[category], defaultdict):
                for key in list(self.metrics[category].keys()):
                    ...
                    while (self.metrics[category][key] and 
                           datetime.fromisoformat(self.metrics[category][key][0]['timestamp']) < cutoff_time):
                        self.metrics[category][key].popleft()
            else:
                # Handle regular deque
                while (self.metrics[category] and 
                       datetime.fromisoformat(self.metrics[category][0]['timestamp']) < cutoff_time):
    ...
    def get_metrics_summary(self, hours: int = 1) -> Dict[str, Any]:
        ...
        if self.metrics['cpu']:
            recent_cpu = [c for c in self.metrics['cpu'] 
    ...
    def export_metrics(self, filepath: str):
        ...
        export_data = {
            ...
            'metrics': {k: list(v) if isinstance(v, deque) else dict(v) for k, v in self.metrics.items()},
            'alerts': self.alerts
        }
...
def record_v4_request(endpoint: str, method: str, response_time_ms: float, 
                    status_code: int, user_id: Optional[str] = None):
    """Record a v4 API request metric."""
    v4_monitor.record_request_metric(endpoint, method, response_time_ms, status_code, user_id)

def record_v4_error(endpoint: str, error_type: str, error_message: str, 
                  user_id: Optional[str] = None):
