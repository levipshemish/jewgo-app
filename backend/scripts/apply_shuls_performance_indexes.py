#!/usr/bin/env python3
"""Script to apply performance indexes for the shuls table.
This script will add the necessary indexes to improve query performance
for the synagogues API, especially location-based queries.
"""
...
def apply_performance_indexes():
    """Apply performance indexes to the shuls table."""
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            trans = conn.begin()
            try:
                logger.info("Starting performance index creation for shuls table")
                ...
                sql_file_path = os.path.join(
                    ...
                    'database', 
                    'migrations', 
                    'add_shuls_performance_indexes.sql'
                )
...
def verify_indexes():
    """Verify that the performance indexes were created successfully."""
    database_url = os.environ.get("DATABASE_URL")
    if not database_url:
        logger.error("DATABASE_URL environment variable is required")
        return False
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            if missing_indexes:
                ...
                logger.info("All critical performance indexes are present")
                return True
...
if __name__ == "__main__":
    logger.info("Starting shuls performance index application...")
    ...
    if apply_performance_indexes():
        ...
        # Verify the indexes
        if verify_indexes():
            logger.info("All performance indexes verified successfully")
            sys.exit(0)
        else:
            logger.error("Index verification failed")
            sys.exit(1)
    else:
        logger.error("Failed to apply performance indexes")
        sys.exit(1)
