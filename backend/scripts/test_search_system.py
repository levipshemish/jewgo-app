...
async def test_search_system():
    ...
    try:
        ...
        for provider_name, status in health_status["providers"].items():
            print(
                f"  {provider_name}: {'✅' if status['healthy'] else '❌'} ({status.get('type', 'unknown')})"
            )
        ...
        for provider_name, info in providers.items():
            print(
                f"  {provider_name}: {'✅' if info['healthy'] else '❌'} ({info['type']})"
            )

        # Test 3: Basic Search
        print("\n🔎 Testing Basic Search...")
        test_queries = ["kosher", "miami", "dairy", "restaurant"]
        ...
        for query in test_queries:
            ...
            try:
                ...
                print(
                    f"    Execution Time: {response.search_metadata.execution_time_ms}ms"
                )

                for i, result in enumerate(response.results[:3], 1):
                    print(
                        f"    {i}. {result.name} ({result.city}) - Score: {result.relevance_score:.2f}"
                    )
        ...
        filters = SearchFilters(kosher_category=KosherCategory.DAIRY, city="Miami")
        ...
        try:
            ...
            for i, result in enumerate(response.results[:3], 1):
                print(
                    f"    {i}. {result.name} ({result.city}) - {result.kosher_category}"
                )
        ...
        try:
            response = await search_service.search(
                "kosher", search_type="postgresql", limit=3
            )
            print(f"    Results: {len(response.results)}")
