...
from utils.feature_flags_v4 import (
    api_v4_flags, get_migration_status, migrate_stage, 
    MigrationStage, enable_testing_mode, enable_partial_rollout,
    enable_full_rollout, complete_migration, rollback_migration
)
...
class APIV4MigrationManager:
    ...
    def get_current_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                logger.warning(f"Failed to get migration status: {response.status_code}")
    ...
    def _test_restaurant_endpoints(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = self.session.get(f"{self.base_url}/api/v4/restaurants/search?q=pizza")
    ...
    def compare_v3_v4_performance(self, endpoint: str, iterations: int = 10) -> Dict[str, Any]:
    ...
    def generate_migration_report(self) -> Dict[str, Any]:
        ...
        for endpoint in key_endpoints:
            performance_results[endpoint] = self.compare_v3_v4_performance(endpoint, iterations=5)
        ...
        report = {
            ...
            'recommendations': self._generate_recommendations(status, test_results, performance_results)
        }
        
        return report
    
    def _generate_recommendations(self, status: Dict, test_results: Dict, performance_results: Dict) -> List[str]:
        ...
        if disabled_count == total_flags:
            recommendations.append("All flags are disabled. Consider enabling testing mode to begin migration.")
        elif disabled_count > total_flags * 0.5:
            recommendations.append("Most flags are still disabled. Consider enabling testing mode for gradual migration.")
        ...
        if test_errors > 0:
            recommendations.append(f"Found {test_errors} test errors. Review and fix before proceeding with migration.")
        
        # Check performance
        for endpoint, perf in performance_results.items():
            if 'v4_avg' in perf and 'v3_avg' in perf:
                if perf['v4_avg'] > perf['v3_avg'] * 1.5:
                    recommendations.append(f"V4 {endpoint} endpoint is significantly slower than V3. Investigate performance issues.")
...
def main():
    ...
    parser.add_argument('--action', choices=['status', 'test', 'testing', 'partial', 'full', 'complete', 'rollback', 'report'], 
                       default='status', help='Action to perform')
    parser.add_argument('--percentage', type=float, default=10.0, help='Rollout percentage for partial migration')
