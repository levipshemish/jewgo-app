...
from utils.feature_flags_v4 import (
    api_v4_flags, MigrationStage, migrate_stage, bulk_migrate_stage,
    enable_testing_mode, enable_partial_rollout, enable_full_rollout
)
...
class GradualRolloutManager:
    ...
    def start_rollout(self) -> Dict[str, Any]:
        ...
        self.rollout_thread = threading.Thread(
            target=self._rollout_monitoring_loop,
            daemon=True
        )
        ...
        while self.rollout_active:
            try:
                ...
                if health_status['is_healthy']:
                    ...
                    if (self.config.enable_auto_increment and 
                        self._should_increment_rollout()):
    ...
    def _evaluate_health(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for endpoint, perf in metrics.get('performance', {}).items():
                ...
                total_response_time += perf.get('avg_response_time_ms', 0) * perf.get('request_count', 0)
            ...
            is_healthy = (
                error_rate < self.config.auto_rollback_threshold and
                len(alerts) == 0 and
                total_requests >= self.config.min_requests_for_evaluation
            )
    ...
    def _should_increment_rollout(self) -> bool:
        ...
        metrics = get_v4_metrics_summary(hours=self.config.monitoring_period_hours)
        total_requests = sum(perf.get('request_count', 0) for perf in metrics.get('performance', {}).values())
        
        if total_requests < self.config.min_requests_for_evaluation:
            logger.info("Insufficient requests for evaluation", 
                       total_requests=total_requests, 
    ...
    def _increment_rollout(self):
        ...
        logger.info("Incrementing rollout percentage", 
    ...
    def _gradual_rollback(self, reason: str):
        ...
        logger.warning(f"Gradual rollback: {reason}", 
    ...
    def _set_rollout_percentage(self, percentage: float):
        ...
        self.status.total_users_affected = int(percentage * 1000)  # Assuming 1000 total users
    ...
    def _update_rollout_status(self):
        ...
        try:
            ...
            self.status.error_rate = (total_errors / total_requests * 100) if total_requests > 0 else 0
            self.status.avg_response_time_ms = (total_response_time / request_count) if request_count > 0 else 0
            self.status.alerts_count = len(get_v4_alerts())
            
            # Determine overall health
            self.status.is_healthy = (
                self.status.error_rate < self.config.auto_rollback_threshold and
                self.status.alerts_count == 0
            )
    ...
    def _generate_recommendations(self, status: Dict, metrics: Dict, alerts: List) -> List[str]:
        """Generate recommendations based on current status."""
        recommendations = []
        
        # Check error rate
        if status['error_rate'] > self.config.auto_rollback_threshold:
            recommendations.append(f"High error rate ({status['error_rate']:.1f}%). Consider rollback.")
        
        # Check response time
        if status['avg_response_time_ms'] > 1000:  # 1 second
            recommendations.append(f"High response time ({status['avg_response_time_ms']:.0f}ms). Consider optimization.")
        
        # Check alerts
        if status['alerts_count'] > 0:
            recommendations.append(f"{status['alerts_count']} active alerts. Review system health.")
        
        # Check rollout progress
        if status['current_percentage'] < 50 and status['is_healthy']:
            recommendations.append("System is healthy. Consider increasing rollout percentage.")
        
        if not recommendations:
            recommendations.append("System is performing well. Continue with rollout plan.")
...
def main():
    ...
    parser.add_argument('--action', choices=['start', 'stop', 'status', 'report', 'force'], 
                       default='status', help='Action to perform')
    parser.add_argument('--percentage', type=float, help='Rollout percentage (for force action)')
