...
def scan_and_fix_restaurant_json_fields(db_manager: DatabaseManager) -> Dict[str, int]:
    ...
    fixes_applied = {"specials": 0, "google_reviews": 0, "total_restaurants_checked": 0}
                ...
                if fixed_specials:
                    try:
                        ...
                        db_manager.restaurant_repo.update_restaurant(
                            restaurant_id, {"specials": fixed_specials}
                        )
                        fixes_applied["specials"] += 1
                        logger.info(
                            f"Fixed specials JSON for restaurant {restaurant_id}"
                        )
                    except Exception as e:
                        logger.error(
                            f"Failed to update specials for restaurant {restaurant_id}: {e}"
                        )
                else:
                    logger.warning(
                        f"Could not fix specials JSON for restaurant {restaurant_id}: {specials[:100]}..."
                    )
            ...
            if google_reviews and not is_valid_json(google_reviews):
                ...
                if fixed_reviews:
                    try:
                        ...
                        logger.info(
                            f"Fixed google_reviews JSON for restaurant {restaurant_id}"
                        )
                    except Exception as e:
                        logger.error(
                            f"Failed to update google_reviews for restaurant {restaurant_id}: {e}"
                        )
                else:
                    logger.warning(
                        f"Could not fix google_reviews JSON for restaurant {restaurant_id}: {google_reviews[:100]}..."
                    )
...
def scan_and_fix_review_json_fields(db_manager: DatabaseManager) -> Dict[str, int]:
    ...
    fixes_applied = {"review_images": 0, "total_reviews_checked": 0}
                ...
                if fixed_images:
                    try:
                        ...
                        db_manager.review_repo.update_review(
                            review_id, {"images": fixed_images}
                        )
                        fixes_applied["review_images"] += 1
                        logger.info(f"Fixed images JSON for review {review_id}")
                    except Exception as e:
                        logger.error(
                            f"Failed to update images for review {review_id}: {e}"
                        )
                else:
                    logger.warning(
                        f"Could not fix images JSON for review {review_id}: {images[:100]}..."
                    )
...
def main():
    ...
    try:
        ...
        logger.info(
            f"Restaurants checked: {restaurant_fixes['total_restaurants_checked']}"
        )
