    ...
    try:
        ...
        for place_data in google_data:
            try:
                ...
                if update_data:
                    if db_manager.update_restaurant_data(
                        place_data.restaurant_id, update_data
                    ):
                        print(
                            f"✅ Updated restaurant {place_data.restaurant_id}: {restaurant.get('name', 'Unknown')}"
                        )
                        success_count += 1
                    else:
                        print(
                            f"❌ Failed to update restaurant {place_data.restaurant_id}"
                        )
                        error_count += 1
                else:
                    print(
                        f"⚠️  No data to update for restaurant {place_data.restaurant_id}"
                    )
