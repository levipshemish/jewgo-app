...
def update_restaurant_hours_direct_sql(restaurant_id, hours_data):
    """Update a restaurant with hours information using direct SQL."""
    ...
    try:
        ...
        if hours_data.get('opening_hours'):
            ...
            update_params['hours_json'] = hours_json
            ...
            if hours_data['opening_hours'].get('weekday_text'):
                ...
                update_params['hours_of_operation'] = hours_text
        
        if hours_data.get('formatted_address'):
            update_params['formatted_address'] = hours_data['formatted_address']
        
        if hours_data.get('website'):
            update_params['website'] = hours_data['website']
        
        # Add Google Place ID if we have it
        if hours_data.get('place_id'):
            update_params['google_place_id'] = hours_data['place_id']
        
        # Add timestamp
        update_params['last_google_sync_at'] = datetime.utcnow()
        update_params['restaurant_id'] = restaurant_id
        
        # Update the restaurant using direct SQL
...
def update_all_restaurants_hours_direct_sql():
    """Update all restaurants with hours information using direct SQL."""
    ...
    for i, restaurant in enumerate(restaurants, 1):
        ...
        try:
            ...
            if hours_data and hours_data.get('opening_hours'):
                ...
                if update_restaurant_hours_direct_sql(restaurant_id, hours_data):
...
if __name__ == "__main__":
    update_all_restaurants_hours_direct_sql()
