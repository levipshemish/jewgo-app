...
def scan_and_fix_python_dicts(db_manager: DatabaseManager) -> Dict[str, int]:
    ...
    try:
        ...
        logger.info(f"Scanning {len(restaurants)} restaurants for Python dict strings in Google reviews")
            ...
            if is_python_dict_string(google_reviews):
                ...
                if fixed_reviews:
                    try:
                        ...
                        logger.info(f"Fixed Python dict string for restaurant {restaurant_id}")
                    except Exception as e:
                        logger.error(f"Failed to update Google reviews for restaurant {restaurant_id}: {e}")
                else:
                    logger.warning(f"Could not convert Python dict string for restaurant {restaurant_id}: {google_reviews[:100]}...")
...
def main():
    ...
    try:
        ...
        logger.info(f"Restaurants with Python dict strings: {fixes['restaurants_with_issues']}")
        logger.info(f"Google reviews fixed: {fixes['google_reviews_fixed']}")
        
        if fixes['google_reviews_fixed'] > 0:
            logger.info("✅ Python dict strings have been converted to JSON successfully")
        else:
