...
def scan_and_fix_google_reviews(db_manager: DatabaseManager) -> Dict[str, int]:
    ...
    try:
        ...
        logger.info(
            f"Scanning {len(restaurants)} restaurants for malformed Google reviews JSON"
        )
            ...
            try:
                ...
                if fixed_reviews:
                    try:
                        ...
                        logger.info(
                            f"Fixed Google reviews JSON for restaurant {restaurant_id}"
                        )
                    except Exception as e:
                        logger.error(
                    ...
                    logger.warning(
                        f"Could not fix Google reviews JSON for restaurant {restaurant_id}: {google_reviews[:100]}..."
                    )
...
def main():
    ...
    try:
        ...
        logger.info(
            f"Restaurants with malformed JSON: {fixes['restaurants_with_issues']}"
        )
