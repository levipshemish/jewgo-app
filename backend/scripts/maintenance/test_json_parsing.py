            ...
            if specials:
                ...
                try:
                    ...
                    if parsed_specials == [] and specials.strip():
                        print(f"⚠️  Restaurant {restaurant_id} ({restaurant_name}): specials field still has issues")
                        json_errors += 1
                except Exception as e:
                    print(f"❌ Restaurant {restaurant_id} ({restaurant_name}): specials parsing error: {e}")
                    json_errors += 1
            ...
            if google_reviews:
                ...
                try:
                    ...
                    if parsed_reviews == [] and google_reviews.strip():
                        print(f"⚠️  Restaurant {restaurant_id} ({restaurant_name}): google_reviews field still has issues")
                        json_errors += 1
                except Exception as e:
                    print(f"❌ Restaurant {restaurant_id} ({restaurant_name}): google_reviews parsing error: {e}")
                    json_errors += 1
        
        print(f"\n=== JSON Parsing Test Results ===")
        print(f"Total fields tested: {total_tested}")
        print(f"JSON parsing errors: {json_errors}")
        print(f"Success rate: {((total_tested - json_errors) / total_tested * 100):.1f}%" if total_tested > 0 else "N/A")
