...
class MonitoringManager:
    ...
    def record_retry(
        self, operation: str, attempt: int, reason: str, context: Optional[Dict] = None
    ):
            ...
            for op in operations:
                ...
                if metric_type == "api_calls":
                    metrics[metric_type][op] = {
                        ...
                        "avg_duration": (
                            sum(e["duration"] for e in recent_events)
                            / len(recent_events)
                            if recent_events
                            else 0
                        ),
                    ...
                    for event in recent_events:
                        metrics[metric_type][op]["status_codes"][
                            event["status_code"]
                        ] += 1
                else:
                    metrics[metric_type][op] = {
...
def record_error(
    operation: str, error_type: str, error_message: str, context: Optional[Dict] = None
):
    """Record an error event."""
...
def record_retry(
    operation: str, attempt: int, reason: str, context: Optional[Dict] = None
):
    """Record a retry event."""
...
def record_api_call(
    operation: str, duration: float, status_code: int, context: Optional[Dict] = None
):
    """Record an API call event."""
...
def get_metrics(
    operation: Optional[str] = None, time_window: Optional[timedelta] = None
) -> Dict[str, Any]:
