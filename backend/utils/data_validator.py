...
class DataValidator:
    ...
    # Valid listing types
    VALID_LISTING_TYPES = ['restaurant', 'catering', 'bakery', 'deli', 'grocery', 'other']
    ...
    def validate_hours_format(hours_data: Any) -> Dict[str, Any]:
        ...
        if isinstance(hours_data, str):
            try:
                ...
                if ":" in hours_data and ("AM" in hours_data.upper() or "PM" in hours_data.upper()):
    ...
    @staticmethod
    def validate_restaurant_data(data: Dict[str, Any], strict: bool = False) -> Dict[str, Any]:
        ...
        if data.get("phone_number") and not DataValidator.validate_phone_number(data["phone_number"]):
            errors.append("Invalid phone number format")
        
        if data.get("zip_code") and not DataValidator.validate_zip_code(data["zip_code"]):
        ...
        if data.get("kosher_category") and not DataValidator.validate_kosher_category(data["kosher_category"]):
            errors.append(f"Invalid kosher category. Must be one of: {', '.join(DataValidator.VALID_KOSHER_CATEGORIES)}")
        
        if data.get("listing_type") and not DataValidator.validate_listing_type(data["listing_type"]):
            errors.append(f"Invalid listing type. Must be one of: {', '.join(DataValidator.VALID_LISTING_TYPES)}")
        
        if data.get("status") and not DataValidator.validate_restaurant_status(data["status"]):
            errors.append(f"Invalid status. Must be one of: {', '.join(DataValidator.VALID_RESTAURANT_STATUSES)}")
        
        # Coordinate validation
        if data.get("latitude") is not None and data.get("longitude") is not None:
            if not DataValidator.validate_coordinates(data["latitude"], data["longitude"]):
        ...
        if errors:
            raise ValidationError(
                f"Restaurant validation failed: {', '.join(errors)}",
                result
            )
    ...
    def validate_review_data(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if data.get("user_email") and not DataValidator.validate_email(data["user_email"]):
            errors.append("Invalid email format")
        ...
        if data.get("images"):
            if not isinstance(data["images"], list):
                ...
                for i, image in enumerate(data["images"]):
                    if not isinstance(image, str) or not DataValidator.validate_url(image):
        ...
        if errors:
            raise ValidationError(
                f"Review validation failed: {', '.join(errors)}",
                result
            )
