...
class BaseRepository(Generic[T]):
    ...
    def __init__(self, connection_manager: DatabaseConnectionManager, model_class: Type[T]):
    ...
    def get_by_id(self, record_id: Any) -> Optional[T]:
        ...
        try:
            ...
            self.logger.exception("Failed to get record by ID", id=record_id, error=str(e))
            return None
    ...
    def exists(self, record_id: Any) -> bool:
        ...
        try:
            session = self.connection_manager.get_session()
            exists = session.query(self.model_class).filter_by(id=record_id).first() is not None
            session.close()
            return exists
        except Exception as e:
            self.logger.exception("Failed to check record existence", id=record_id, error=str(e))
            return False
    ...
    def find_by_field(self, field: str, value: Any) -> Optional[T]:
        ...
        try:
            ...
            session = self.connection_manager.get_session()
            instance = session.query(self.model_class).filter_by(**{field: value}).first()
            session.close()
            return instance
        except Exception as e:
            self.logger.exception("Failed to find record by field", field=field, value=value, error=str(e))
            return None
    ...
    def find_all_by_field(self, field: str, value: Any) -> List[T]:
        ...
        try:
            ...
            session = self.connection_manager.get_session()
            instances = session.query(self.model_class).filter_by(**{field: value}).all()
            session.close()
            return instances
        except Exception as e:
            self.logger.exception("Failed to find records by field", field=field, value=value, error=str(e))
            return []
