...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "price_cents", sa.Integer, nullable=False, default=0
        ),  # Price in cents (0 for Gemach)
        ...
        sa.Column(
            "category_name", sa.String(100), nullable=False
        ),  # Judaica, Holiday Items, etc.
        sa.Column(
            "subcategory", sa.String(100), nullable=True
        ),  # Mezuzot, Passover, etc.
        # 🖼️ Images
        ...
        sa.Column(
            "seller_user_id", sa.String(100), nullable=True
        ...
        sa.Column(
            "is_gemach", sa.Boolean, default=False, nullable=False
        ),  # Free community loan
        sa.Column(
            "gemach_type", sa.String(50), nullable=True
        ),  # baby_items, books, etc.
        sa.Column(
            "loan_duration_days", sa.Integer, nullable=True
        ),  # Expected loan duration
        sa.Column(
            "return_condition", sa.String(200), nullable=True
        ),  # Return conditions
        # 🗓️ Jewish Calendar Features
        sa.Column(
            "holiday_category", sa.String(50), nullable=True
        ),  # passover, sukkot, etc.
        sa.Column("seasonal_item", sa.Boolean, default=False, nullable=False),
        sa.Column(
            "available_until", sa.Date, nullable=True
        ),  # Expiration for seasonal items
        # 📊 Item Metadata
        sa.Column(
            "condition", sa.String(20), default="good", nullable=False
        ...
        ),  # active, sold, loaned, inactive
        sa.Column(
            "transaction_type", sa.String(20), default="sale", nullable=False
        ),  # sale, gemach, trade
        ...
        sa.Column(
            "pickup_instructions", sa.Text, nullable=True
        ),  # How to arrange pickup/delivery
        sa.Column("notes", sa.Text, nullable=True),  # Additional notes
        sa.Column(
            "contact_preference", sa.String(20), default="phone", nullable=False
        ...
        sa.CheckConstraint(
            "rating >= 0 AND rating <= 5", name="check_shtetl_rating_range"
        ),
        sa.CheckConstraint(
    ...
    op.execute(
        "COMMENT ON COLUMN shtetl_marketplace.id IS 'Unique shtetl item identifier'"
    )
    op.execute("COMMENT ON COLUMN shtetl_marketplace.title IS 'Item title (required)'")
    op.execute(
        "COMMENT ON COLUMN shtetl_marketplace.price_cents IS 'Price in cents, 0 for Gemach items'"
    )
    op.execute(
        "COMMENT ON COLUMN shtetl_marketplace.is_gemach IS 'True if this is a free community loan item'"
    )
    op.execute(
        "COMMENT ON COLUMN shtetl_marketplace.kosher_agency IS 'Kosher certification agency (OU, OK, etc.)'"
    )
    op.execute(
        "COMMENT ON COLUMN shtetl_marketplace.community_verified IS 'Verified by community members'"
    )
    op.execute(
        "COMMENT ON COLUMN shtetl_marketplace.holiday_category IS 'Jewish holiday category (passover, sukkot, etc.)'"
    )
    op.execute(
        "COMMENT ON COLUMN shtetl_marketplace.transaction_type IS 'sale, gemach, trade, or donation'"
    )
