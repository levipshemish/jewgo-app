...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("title", sa.String(500), nullable=False),  # Product title/display name
        ...
        # 🖼️ Product Images
        sa.Column("product_image", sa.String(2000), nullable=True),  # Main product image
        sa.Column("additional_images", ARRAY(sa.String), nullable=True),  # Additional product images
        ...
        sa.Column("original_price", sa.Numeric(10, 2), nullable=True),  # Original price for sales
        ...
        sa.Column("discount_percentage", sa.Integer, nullable=True),  # Discount percentage
        ...
        sa.Column("kosher_agency", sa.String(100), nullable=True),  # Kosher certification agency
        sa.Column("kosher_level", sa.String(50), nullable=True),  # Glatt, regular, chalav_yisrael, pas_yisrael
        ...
        # 📊 Business Logic
        sa.Column("status", sa.String(20), default="active", nullable=False),  # active, inactive, pending, sold_out
        sa.Column("priority", sa.Integer, default=0, nullable=False),  # For featured/sorting
        sa.Column("expiry_date", sa.Date, nullable=True),  # Product expiry date
        sa.Column("created_by", sa.String(100), nullable=True),  # Who created the listing
        sa.Column("approved_by", sa.String(100), nullable=True),  # Who approved the listing
        ...
        sa.Column("source", sa.String(50), default="manual", nullable=False),  # manual, import, api
        ...
        sa.CheckConstraint("status IN ('active', 'inactive', 'pending', 'sold_out')", name="check_status_valid"),
        sa.CheckConstraint("kosher_level IN ('glatt', 'regular', 'chalav_yisrael', 'pas_yisrael')", name="check_kosher_level_valid"),
        ...
        sa.Index("idx_marketplace_search", "name", "title", "description", postgresql_using="gin"),
    )
    
    # Add table and column comments
    op.execute("COMMENT ON TABLE marketplace IS 'Marketplace product listings with kosher certification and vendor information'")
    ...
    op.execute("COMMENT ON COLUMN marketplace.title IS 'Product title/display name (required)'")
    op.execute("COMMENT ON COLUMN marketplace.price IS 'Product price in USD (required)'")
    op.execute("COMMENT ON COLUMN marketplace.category IS 'Main product category (required)'")
    op.execute("COMMENT ON COLUMN marketplace.location IS 'Product location/address (required)'")
    op.execute("COMMENT ON COLUMN marketplace.product_image IS 'Main product image URL'")
    ...
    op.execute("COMMENT ON COLUMN marketplace.vendor_name IS 'Vendor/store name (required)'")
    op.execute("COMMENT ON COLUMN marketplace.kosher_agency IS 'Kosher certification agency'")
    op.execute("COMMENT ON COLUMN marketplace.kosher_level IS 'Kosher certification level'")
    op.execute("COMMENT ON COLUMN marketplace.status IS 'Product status: active, inactive, pending, sold_out'")
    ...
    op.execute("COMMENT ON COLUMN marketplace.is_available IS 'Whether product is available for purchase'")
    op.execute("COMMENT ON COLUMN marketplace.is_featured IS 'Whether product is featured'")
    op.execute("COMMENT ON COLUMN marketplace.is_on_sale IS 'Whether product is on sale'")
    op.execute("COMMENT ON COLUMN marketplace.discount_percentage IS 'Discount percentage (0-100)'")
