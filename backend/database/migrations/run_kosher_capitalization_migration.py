...
def run_complete_migration() -> bool:
    ...
    script_dir = os.path.dirname(__file__)
    data_migration_script = os.path.join(script_dir, "update_kosher_types_capitalization.py")
    ...
    constraint_migration_script = os.path.join(script_dir, "update_kosher_category_constraints.py")
    
    if not run_migration_script(constraint_migration_script, "Constraint Update Migration"):
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Kosher Types and Text Capitalization Migration")
    parser.add_argument(
        "--status", 
        action="store_true", 
