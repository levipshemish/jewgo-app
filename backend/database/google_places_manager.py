...
class GooglePlacesManager:
    ...
    def __init__(self, database_url: str | None = None) -> None:
        ...
        if not self.api_key:
            logger.warning(
                "GOOGLE_PLACES_API_KEY not set - some features will be limited",
            )
    ...
    def connect(self) -> bool:
        """Connect to the database with a resilient engine configuration."""
        try:
            ...
            self.engine = create_engine(
                ...
                connect_args={
                    "connect_timeout": 30,
                    "application_name": "jewgo-google-places",
                },
            )
    ...
    def store_place_data(
        ...
        try:
            ...
            existing = (
                session.query(GooglePlacesData)
                .filter_by(google_place_id=place_id)
                .first()
            )
            ...
            if existing:
                ...
                logger.info(
                    "Updated Google Places data for restaurant",
                ...
                logger.info(
                    "Stored new Google Places data for restaurant",
                    restaurant_id=restaurant_id,
                )
            ...
            logger.exception(
                "Error storing place data for restaurant",
                restaurant_id=restaurant_id,
                error=str(e),
            )
            if session:
    ...
    def _update_place_data(
        self,
        existing_data: GooglePlacesData,
        place_data: dict[str, Any],
    ) -> None:
        ...
        existing_data.formatted_address = result.get(
            "formatted_address",
            existing_data.formatted_address,
        )
        existing_data.phone_number = result.get(
            "formatted_phone_number",
            existing_data.phone_number,
        )
        ...
        existing_data.user_ratings_total = result.get(
            "user_ratings_total",
            existing_data.user_ratings_total,
        )
        existing_data.price_level = result.get("price_level", existing_data.price_level)

        # Update location if available
        if "geometry" in result and "location" in result["geometry"]:
            existing_data.latitude = result["geometry"]["location"].get(
                "lat",
                existing_data.latitude,
            )
            existing_data.longitude = result["geometry"]["location"].get(
                "lng",
                existing_data.longitude,
            )
        ...
        existing_data.primary_photo_url = (
            self._get_primary_photo_url(result.get("photos"))
            or existing_data.primary_photo_url
        )
        existing_data.types_json = result.get("types", existing_data.types_json)
        existing_data.opening_hours_json = result.get(
            "opening_hours",
            existing_data.opening_hours_json,
        )
        existing_data.reviews_json = result.get("reviews", existing_data.reviews_json)

        # Update metadata
        existing_data.last_updated = datetime.utcnow()
        existing_data.next_update = datetime.utcnow() + timedelta(
            hours=existing_data.update_frequency_hours,
        )
    ...
    def _format_hours_text(self, opening_hours: dict[str, Any]) -> str:
        ...
        return HoursFormatter.to_text(opening_hours)
    ...
    def get_place_data(self, restaurant_id: int) -> dict[str, Any] | None:
        ...
        try:
            ...
            logger.exception(
                "Error getting place data for restaurant",
                restaurant_id=restaurant_id,
                error=str(e),
            )
            return None
    ...
    def _place_data_to_dict(self, place_data: GooglePlacesData) -> dict[str, Any]:
        ...
        return {
            ...
            "last_updated": (
                place_data.last_updated.isoformat() if place_data.last_updated else None
            ),
            "next_update": (
                place_data.next_update.isoformat() if place_data.next_update else None
            ),
    ...
    def get_places_needing_update(self, limit: int = 50) -> list[GooglePlacesData]:
        ...
        try:
            ...
            logger.exception("Error getting places needing update", error=str(e))
            return []
        finally:
            if session:
                session.close()
    ...
    def update_place_from_api(self, place_data: GooglePlacesData) -> bool:
        ...
        try:
            ...
            if data["status"] == "OK":
                ...
                logger.info(
                    "Successfully updated Google Places data",
                    google_place_id=place_data.google_place_id,
                )
                return True
            # Handle API errors
            ...
            if place_data.error_count > 3:
                place_data.update_frequency_hours = min(
                    place_data.update_frequency_hours * 2,
                    1680,
                )  # Max 1 week
            ...
            logger.warning(
                "Failed to update Google Places data",
                google_place_id=place_data.google_place_id,
        ...
        try:
            ...
            for place_data in places_to_update:
                ...
                if place_data.error_count > 5:
                    ...
                    logger.warning(
                        "Skipping update due to high error count",
                        google_place_id=place_data.google_place_id,
                    )
                    continue
            ...
            logger.exception("Error running periodic updates", error=str(e))
            return stats
    ...
    def cleanup_old_data(self, days_old: int = 7) -> int:
        ...
        try:
            ...
            logger.info("Marked old Google Places records as inactive", count=result)
            return result

        except Exception as e:
            logger.exception("Error cleaning up old data", error=str(e))
            return 0
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        try:
            ...
            active_records = (
                session.query(GooglePlacesData).filter_by(is_active=True).count()
            )
            ...
            error_records = (
                session.query(GooglePlacesData)
                .filter(GooglePlacesData.error_count > 0)
                .count()
            )
