...
class ShtetlMessageService:
        ...
        try:
            ...
            if not message_data.thread_id:
                if message_data.parent_message_id:
                    ...
                    parent_query = (
                        "SELECT thread_id FROM shtetl_messages WHERE message_id = %s"
                    )
                    parent_result = self.db_manager.execute_query(
                        parent_query, (message_data.parent_message_id,), fetch_one=True
                    )
                    if parent_result:
            ...
            if (
                not message_data.store_id
                or not message_data.sender_user_id
                or not message_data.recipient_user_id
            ):
                return (
                    False,
                    "Store ID, sender user ID, and recipient user ID are required",
                    None,
                )
            ...
            params = (
                ...
                message_data.kosher_related,
                message_data.kosher_question_type,
                message_data.hechsher_inquiry,
                message_data.shabbos_related,
                message_data.holiday_related,
                message_data.community_question,
                message_data.category,
                json.dumps(message_data.tags or []),
                message_data.keywords,
                message_data.has_attachments,
                json.dumps(message_data.attachment_urls or []),
                json.dumps(message_data.attachment_types or []),
                message_data.scheduled_send_time,
                message_data.is_scheduled,
                message_data.timezone,
                message_data.source,
                message_data.user_agent,
                message_data.ip_address,
                json.dumps(message_data.location_data or {}),
            )
            ...
            logger.info(
                f"Created message {message_id} from {message_data.sender_user_id} to {message_data.recipient_user_id}"
            )
    ...
    def get_message_thread(
        ...
        try:
            ...
            """
            results = self.db_manager.execute_query(
                query, (thread_id, limit, offset), fetch_all=True
            )
            # Cache the result
            self.cache_manager.set(cache_key, results, ttl=300)  # 5 minutes
            return results if results else []
    ...
    def mark_message_read(self, message_id: str, user_id: str) -> bool:
        ...
        try:
            ...
            # Clear cache
            self.cache_manager.delete_pattern("user_messages:*")
            self.cache_manager.delete_pattern("store_messages:*")
    ...
    def get_store_analytics(
        ...
        try:
            ...
            unread_result = self.db_manager.execute_query(
                unread_query, tuple(unread_params), fetch_one=True
            )
            if unread_result:
            ...
            urgent_result = self.db_manager.execute_query(
                urgent_query, tuple(urgent_params), fetch_one=True
            )
            if urgent_result:
            ...
            type_results = self.db_manager.execute_query(
                type_query, tuple(type_params), fetch_all=True
            )
            ...
            self.logger.error(
                f"Error getting message analytics for store {store_id}: {e}"
            )
    ...
    def _update_reply_count(self, parent_message_id: str) -> None:
        ...
        try:
            ...
            self.logger.error(
                f"Error updating reply count for message {parent_message_id}: {e}"
            )
