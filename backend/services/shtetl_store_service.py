...
class ShtetlStoreService:
    ...
    def create_store(self, store_data: StoreData) -> Tuple[bool, str, Optional[Dict]]:
        ...
        try:
            ...
            params = (
                ...
                store_data.subcategory,
                store_data.owner_name,
                store_data.owner_email,
                store_data.owner_phone,
                store_data.address,
                store_data.city,
                store_data.state,
                store_data.zip_code,
                ...
                store_data.email,
                store_data.website,
                store_data.business_hours,
                store_data.timezone,
                store_data.delivery_enabled,
                store_data.delivery_radius_miles,
                store_data.delivery_fee,
                store_data.delivery_minimum,
                store_data.pickup_enabled,
                store_data.kosher_certification,
                store_data.kosher_agency,
                store_data.kosher_level,
                store_data.is_cholov_yisroel,
                store_data.is_pas_yisroel,
                store_data.shabbos_orders,
                store_data.shabbos_delivery,
                store_data.logo_url,
                store_data.banner_url,
                store_data.color_scheme,
                store_data.custom_domain,
                store_data.plan_type,
                store_data.stripe_customer_id,
                store_data.stripe_subscription_id,
                store_data.is_active,
                store_data.is_approved,
                store_data.status,
    ...
    def get_store_analytics(
        self, store_id: str, period: str = "total"
    ) -> Optional[StoreAnalytics]:
        """Get store analytics."""
        try:
            # Check cache first
            ...
            if products_result:
                ...
                analytics.active_products = int(
                    products_result.get("active_products", 0)
                )
                analytics.featured_products = int(
                    products_result.get("featured_products", 0)
                )
    ...
    def get_store_products(
        self, store_id: str, limit: int = 50, offset: int = 0
    ) -> List[Dict]:
    ...
    def check_plan_limits(self, store_id: str, operation: str) -> Tuple[bool, str]:
        ...
        try:
            ...
            if operation == "add_product":
                ...
                messages_query = (
                    "SELECT COUNT(*) as count FROM shtetl_messages WHERE store_id = %s"
                )
                result = self.db_manager.execute_query(
