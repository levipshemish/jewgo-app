...
class ShtetlOrderService:
    ...
    def create_order(self, order_data: OrderData) -> Tuple[bool, str, Optional[str]]:
        ...
        try:
            ...
            if order_data.subtotal == 0.0:
                order_data.subtotal = sum(item.get('price', 0) * item.get('quantity', 1) 
                                        for item in order_data.order_items)
            
            if order_data.total_amount == 0.0:
                order_data.total_amount = (order_data.subtotal + order_data.tax_amount + 
                                         order_data.delivery_fee - order_data.discount_amount)
            ...
            params = (
                order_data.order_id, order_data.store_id, order_data.store_name,
                order_data.customer_user_id, order_data.customer_name, order_data.customer_email,
                order_data.customer_phone, json.dumps(order_data.order_items), order_data.subtotal,
                order_data.tax_amount, order_data.delivery_fee, order_data.discount_amount,
                order_data.total_amount, order_data.delivery_address, order_data.delivery_city,
                order_data.delivery_state, order_data.delivery_zip_code, order_data.delivery_instructions,
                order_data.payment_method, order_data.payment_status, order_data.payment_transaction_id,
                order_data.stripe_payment_intent_id, order_data.order_status, order_data.fulfillment_method,
                order_data.estimated_delivery_date, order_data.actual_delivery_date, order_data.tracking_number,
                order_data.carrier, order_data.kosher_certification_required, order_data.kosher_agency_preference,
                order_data.is_cholov_yisroel, order_data.is_pas_yisroel, order_data.shabbos_delivery,
                order_data.holiday_delivery, order_data.special_instructions, order_data.customer_notes,
                order_data.store_notes, order_data.source, order_data.referral_code, order_data.campaign_id
            )
    ...
    def get_store_orders(self, store_id: str, limit: int = 50, offset: int = 0, 
    ...
    def get_customer_orders(self, customer_user_id: str, limit: int = 50, offset: int = 0) -> List[Dict]:
        ...
        try:
            ...
            self.logger.error(f"Error getting orders for customer {customer_user_id}: {e}")
            return []
    
    def update_order_status(self, order_id: str, status: str, notes: Optional[str] = None) -> bool:
        """Update order status."""
        try:
            query = """
    ...
    def update_payment_status(self, order_id: str, payment_status: str, 
        ...
        try:
            ...
            self.db_manager.execute_query(query, (payment_status, transaction_id, order_id))
    ...
    def get_store_analytics(self, store_id: str, start_date: Optional[datetime] = None, 
                           end_date: Optional[datetime] = None) -> OrderAnalytics:
        ...
        try:
            ...
            if total_result:
                ...
                analytics.average_order_value = float(total_result.get('average_order_value', 0))
            ...
            status_results = self.db_manager.execute_query(status_query, tuple(status_params), fetch_all=True)
            ...
            delivery_results = self.db_manager.execute_query(delivery_query, tuple(delivery_params), fetch_all=True)
    ...
    def search_orders(self, store_id: str, search_term: str, limit: int = 50, offset: int = 0) -> List[Dict]:
