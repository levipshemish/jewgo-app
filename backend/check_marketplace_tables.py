...
def check_marketplace_tables():
    ...
    try:
        ...
        try:
            ...
            if count > 0:
                cursor.execute('SELECT id, title, type, price_cents, status, created_at FROM "Marketplace listings" LIMIT 3')
        ...
        try:
            ...
            if count > 0:
                cursor.execute('SELECT id, title, name, price, category, status, created_at FROM marketplace LIMIT 3')
                items = cursor.fetchall()
                print('📝 Sample items:')
                for item in items:
                    print(f'  ID: {item["id"]}')
