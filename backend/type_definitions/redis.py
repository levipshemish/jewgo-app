...
from typing import (
    Any, Dict, List, Optional, Union, Protocol, TypedDict, 
...
class CacheEntry(Generic[T]):
    ...
    def time_to_live(self) -> int:
        ...
        if self.is_expired():
            return 0
        return int((self.timestamp + timedelta(seconds=self.ttl) - datetime.now()).total_seconds())
...
class SessionManagerProtocol(Protocol):
    ...
    def create_session(self, user_id: Optional[str], data: Dict[str, Any], ttl: int = 3600) -> str:
        """Create new session."""
        ...
...
class RateLimiterProtocol(Protocol):
    ...
    def record_request(self, identifier: str, config: RateLimitConfig) -> RateLimitResult:
        """Record a request."""
        ...
