    ...
    try:
        ...
        if not listings_exists:
            ...
            if count > 0:
                # Show sample data
                cursor.execute(
                    "SELECT id, title, type, price_cents, status, created_at FROM listings LIMIT 5"
                )
        ...
        if not marketplace_exists:
            ...
            if count > 0:
                # Show sample data
                cursor.execute(
                    "SELECT id, name, title, price, category, status, created_at FROM marketplace LIMIT 5"
                )
                sample_data = cursor.fetchall()
