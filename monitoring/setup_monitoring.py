...
class MonitoringSetup:
    ...
    def setup_uptimerobot(self):
        ...
        for monitor in config["monitors"]:
            monitor["url"] = monitor["url"].replace(
                "https://jewgo.onrender.com", self.api_url
            )
            monitor["url"] = monitor["url"].replace(
                "https://jewgo.com", self.frontend_url
            )
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if result.get("stat") == "ok":
                        ...
                        print(
                            f"❌ Failed to create monitor {monitor['name']}: {result.get('error', {}).get('message')}"
                        )
                else:
                    print(
                        f"❌ HTTP error creating monitor {monitor['name']}: {response.status_code}"
                    )
    ...
    def setup_cronitor(self):
        ...
        for monitor in config["monitors"]:
            ...
            try:
                response = requests.post(
                    "https://cronitor.io/api/monitors", headers=headers, json=data
                )
                ...
                if response.status_code in [200, 201]:
                    ...
                    print(
                        f"❌ Failed to create Cronitor monitor {monitor['display_name']}: {response.status_code}"
                    )

            except Exception as e:
                print(
                    f"❌ Error creating Cronitor monitor {monitor['display_name']}: {e}"
                )
