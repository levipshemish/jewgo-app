...
def main():
    ...
    endpoints = [
        {"url": f"{api_url}/health", "name": "Health Check", "expected": 200},
        {"url": f"{api_url}/ping", "name": "Ping", "expected": 200},
        {
            "url": f"{api_url}/api/restaurants?limit=1",
            "name": "Restaurants API",
            "expected": 200,
        },
        {"url": frontend_url, "name": "Frontend", "expected": 200},
    ]

    results = []
    for endpoint in endpoints:
        success = test_endpoint(endpoint["url"], endpoint["name"], endpoint["expected"])
