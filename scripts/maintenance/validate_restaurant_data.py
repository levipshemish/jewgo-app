...
class RestaurantDataValidator:
    ...
    def search_google_knowledge_graph(
        self, restaurant_name: str, city: str, state: str
    ) -> Optional[Dict]:
    ...
    def analyze_data_quality(self, restaurant: Dict) -> Dict:
        ...
        if not restaurant.get("website") or restaurant["website"] in [
            "null",
            "",
            "http://Www.google.com",
        ]:
        ...
        if (
            not restaurant.get("hours_of_operation")
        ...
        if (
            not restaurant.get("short_description")
            or len(restaurant.get("short_description", "")) < 50
        ):
        ...
        if restaurant.get("website") and "google.com" in restaurant["website"].lower():
            issues.append(
                "Website appears to be a Google search result, not actual website"
            )
    ...
    def validate_restaurants(self, limit: int = 10) -> List[Dict]:
        ...
        for restaurant in restaurants:
            ...
            if quality_analysis["potential_improvements"]:
                print(
                    f"⚠️  Improvements: {', '.join(quality_analysis['potential_improvements'])}"
                )

            print(
                f"📊 Data Quality Score: {quality_analysis['data_quality_score']}/100"
            )
    ...
    def generate_validation_report(self, validation_results: List[Dict]) -> str:
        ...
        for result in validation_results:
            report.append(
                f"### {result['restaurant_name']} (ID: {result['restaurant_id']})"
            )
            ...
            if result.get("google_kg_result"):
                report.append(
                    f"- Google Knowledge Graph: {result['google_kg_result']['search_url']}"
                )
