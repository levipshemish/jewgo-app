...
class RestaurantDataUpdater:
    ...
    def get_restaurants_needing_updates(self) -> Dict[str, List[Dict]]:
        ...
        for restaurant in restaurants:
            ...
            if restaurant["website"] and (
                "google.com" in restaurant["website"].lower()
                or restaurant["website"] in ["null", ""]
            ):
                updates_needed["invalid_websites"].append(restaurant_dict)

            # Check for missing hours
            if (
                not restaurant["hours_of_operation"]
                or restaurant["hours_of_operation"] == "Hours not available"
                or restaurant["hours_of_operation"] == "None"
            ):
            ...
            if (
                not restaurant["short_description"]
                or len(restaurant["short_description"]) < 50
                or "Authentic Kosher Restaurant" in restaurant["short_description"]
            ):
    ...
    def generate_google_search_url(
        self, restaurant: Dict, search_type: str = "general"
    ) -> str:
    ...
    def update_restaurant_description(
        self, restaurant_id: int, new_description: str
    ) -> bool:
    ...
    def print_update_plan(self, updates_needed: Dict[str, List[Dict]]):
        ...
        for update_type, restaurants in updates_needed.items():
            if restaurants:
                print(
                    f"🔧 {update_type.replace('_', ' ').title()}: {len(restaurants)} restaurants"
                )
                for restaurant in restaurants[:3]:  # Show first 3
                    ...
                    if update_type == "invalid_websites":
                        ...
                        print(
                            f"     Search: {self.generate_google_search_url(restaurant, 'website')}"
                        )
                    elif update_type == "missing_hours":
                        print(f"     Current: {restaurant['hours_of_operation']}")
                        print(
                            f"     Search: {self.generate_google_search_url(restaurant, 'hours')}"
                        )
                    elif update_type == "missing_phone":
                        print(f"     Current: {restaurant['phone_number']}")
                        print(
                            f"     Search: {self.generate_google_search_url(restaurant, 'phone')}"
                        )
    ...
    def generate_update_instructions(
        self, updates_needed: Dict[str, List[Dict]]
    ) -> str:
        ...
        for update_type, restaurants in updates_needed.items():
            if restaurants:
                ...
                for restaurant in restaurants[:20]:  # Top 20
                    instructions.append(
                        f"### {restaurant['name']} (ID: {restaurant['id']})"
                    )

                    if update_type == "invalid_websites":
                        instructions.append(
                            f"- **Current Website**: {restaurant['website']}"
                        )
                        instructions.append(
                            f"- **Google Search**: {self.generate_google_search_url(restaurant, 'website')}"
                        )
                        instructions.append(
                        ...
                        instructions.append(
                            f"- **Current Hours**: {restaurant['hours_of_operation']}"
                        )
                        instructions.append(
                            f"- **Google Search**: {self.generate_google_search_url(restaurant, 'hours')}"
                        )
                        instructions.append(
                        ...
                        instructions.append(
                            f"- **Current Phone**: {restaurant['phone_number']}"
                        )
                        instructions.append(
                            f"- **Google Search**: {self.generate_google_search_url(restaurant, 'phone')}"
                        )
                        instructions.append(
                        ...
                        current_desc = (
                            restaurant["short_description"][:100] + "..."
                            if len(restaurant["short_description"]) > 100
                            else restaurant["short_description"]
                        )
                        instructions.append(
                            f"- **Current Description**: {current_desc}"
                        )
                        instructions.append(
                            f"- **Google Search**: {self.generate_google_search_url(restaurant)}"
                        )
                        instructions.append(
                        ...
                        instructions.append(
                            f"- **Current Image**: {restaurant['image_url']}"
                        )
                        instructions.append(
                            f"- **Google Search**: {self.generate_google_search_url(restaurant)}"
                        )
