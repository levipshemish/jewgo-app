...
class MenuPricingUpdater:
    ...
    def generate_sample_pricing(
        self, restaurant_name: str, listing_type: str, kosher_category: str
    ) -> Dict[str, Any]:
        ...
        if kosher_category == "meat":
            variation_factor = (
                1.1  # Meat restaurants tend to be slightly more expensive
            )
    ...
    def update_restaurant_pricing(
        self, restaurant_id: int, pricing_data: Dict[str, Any]
    ) -> bool:
    ...
    def process_restaurant(
        self, restaurant_id: int, name: str, listing_type: str, kosher_category: str
    ) -> bool:
        ...
        try:
            ...
            pricing_data = self.generate_sample_pricing(
                name, listing_type, kosher_category
            )
            ...
            if success:
                ...
                logger.info(
                    f"   Sample: {first_section} - ${sample_prices['min']}-${sample_prices['max']} (avg: ${sample_prices['avg']})"
                )
            else:
    ...
    def run(self, limit: int = None):
        ...
        try:
            ...
            for i, (restaurant_id, name, listing_type, kosher_category) in enumerate(
                restaurants, 1
            ):
                logger.info(f"Progress: {i}/{total_count}")

                success = self.process_restaurant(
                    restaurant_id, name, listing_type, kosher_category
                )
                if success:
                    success_count += 1

            logger.info(
                f"✅ Completed! Updated {success_count}/{total_count} restaurants"
            )
