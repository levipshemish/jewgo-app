...
class GooglePlacesDescriptionUpdater:
    ...
    def search_place(self, restaurant_name: str, address: str) -> Optional[str]:
        ...
        return searcher.search_place(restaurant_name, address, search_type="simple")
    ...
    def get_place_details(self, place_id: str) -> Optional[Dict[str, Any]]:
        ...
        return searcher.get_place_details(place_id, fields)

    def generate_description(
        self, place_details: Dict[str, Any], restaurant_name: str
    ) -> str:
        """
        ...
        try:
            ...
            if "editorial_summary" in place_details and place_details[
                "editorial_summary"
            ].get("overview"):
            ...
            if "types" in place_details:
                ...
                if meaningful_types:
                    ...
                    type_display = (
                        ", ".join(meaningful_types[:3]).replace("_", " ").title()
                    )
            ...
            if not description_parts:
                description_parts.append(
                    f"Kosher {restaurant_name.lower()} establishment."
                )
    ...
    def update_restaurant_description(self, business_id: str, description: str) -> bool:
        ...
        try:
            ...
            if success:
                ...
                logger.warning(
                    f"Failed to update description for restaurant {business_id}"
                )
    ...
    def process_restaurant(self, restaurant: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if not restaurant_name or not address:
                logger.warning(
                    f"Skipping restaurant {business_id}: missing name or address"
                )
                return False
            ...
            if current_description and len(current_description) > 50:
                logger.info(
                    f"Restaurant {restaurant_name} already has a substantial description, skipping"
                )
                return True
            ...
            logger.error(
                f"Error processing restaurant {restaurant.get('name', 'Unknown')}: {e}"
            )
            return False
    ...
    def update_all_restaurants(self, limit: int = None) -> Dict[str, int]:
        ...
        try:
            ...
            for i, restaurant in enumerate(restaurants, 1):
                logger.info(
                    f"Processing {i}/{len(restaurants)}: {restaurant.get('name', 'Unknown')}"
                )
            ...
            logger.info(
                f"Update complete. Success: {success_count}, Failed: {failed_count}, Skipped: {skipped_count}"
            )
