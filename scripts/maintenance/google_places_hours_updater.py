...
class GoogleHoursUpdater:
    ...
    def search_google_places(self, query: str, location: str = None) -> Optional[Dict]:
        ...
        searcher = GooglePlacesSearcher(GOOGLE_PLACES_API_KEY)
        place_id = searcher.search_place(query, search_type="simple")
    ...
    def format_hours_from_places_api(self, opening_hours: Dict) -> str:
        ...
        return HoursFormatter.from_google_places(opening_hours)
    ...
    def process_restaurant(self, restaurant: Dict) -> bool:
        ...
        if (
            existing_hours_open
            and existing_hours_open != "None"
            and existing_hours_open != ""
        ):
            print(f"  ℹ️  Already has hours_open: {existing_hours_open[:50]}...")
            # If we have hours_open but not hours_of_operation, copy it
            if (
                not existing_hours_of_operation
                or existing_hours_of_operation == "None"
                or existing_hours_of_operation == ""
            ):
                print(f"  📝 Copying hours_open to hours_of_operation")
                self.update_restaurant_hours(
                    restaurant["id"], existing_hours_open, existing_hours_open
                )
                return True

        if (
            existing_hours_of_operation
            and existing_hours_of_operation != "None"
            and existing_hours_of_operation != ""
        ):
            print(
                f"  ℹ️  Already has hours_of_operation: {existing_hours_of_operation[:50]}..."
            )
            # If we have hours_of_operation but not hours_open, copy it
            if (
                not existing_hours_open
                or existing_hours_open == "None"
                or existing_hours_open == ""
            ):
        ...
        if kg_result:
            print(
                f"  📊 Found Knowledge Graph result: {kg_result.get('name', 'Unknown')}"
            )
