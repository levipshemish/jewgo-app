...
def extract_agency_from_link(cert_link):
    ...
    if "orbkosher.com" in cert_link:
        ...
        if filename and "." in filename:
            ...
            if any(
                agency in name_without_ext.upper()
                for agency in [
                    "ORB",
                    "OU",
                    "CRC",
...
def add_certifying_agency_column():
    ...
    logger.info("Starting certifying_agency column addition")
    ...
    try:
        ...
        with db_manager.session_scope() as session:
            ...
            try:
                ...
                if not result.fetchone():
                    ...
                    session.execute(
                        text(
                            "ALTER TABLE restaurants ADD COLUMN certifying_agency VARCHAR(100)"
                        )
                    )
            ...
            logger.info(
                f"Found {len(restaurants_with_links)} restaurants with kosher_cert_link"
            )
            ...
            for restaurant in restaurants_with_links:
                ...
                logger.info(
                    f"Updated restaurant {name} (ID: {restaurant_id}) with agency: {agency}"
                )
                updated_count += 1
            ...
            logger.info(
                f"Found {len(restaurants_without_links)} restaurants without kosher_cert_link"
            )
            ...
            for restaurant in restaurants_without_links:
                ...
                logger.info(
                    f"Set default agency 'ORB' for restaurant {name} (ID: {restaurant_id})"
                )
                updated_count += 1
        ...
        logger.info(
            "Certifying agency column addition and population completed successfully"
        )
        return True
    ...
    try:
        ...
        with db_manager.session_scope() as session:
            ...
            for name, agency, cert_link in result.fetchall():
                print(
                    f"  {name}: {agency} (from: {cert_link[:50]}{'...' if len(cert_link) > 50 else ''})"
                )
