#!/usr/bin/env python3
"""
Accurate Import Path Fix Script for JewGo App
============================================

This script fixes import path issues with accurate depth calculation by analyzing actual file structure.
"""

import re
from pathlib import Path

def calculate_accurate_path(file_path: Path) -> str:
    """Calculate the accurate relative path to lib based on actual file location."""
...
def fix_import_paths_accurately():
    """Fix import path issues with accurate depth calculation."""
    ...
    for ts_file in api_dir.rglob("*.ts"):
        if ts_file.is_file():
            try:
                ...
                # Calculate accurate path for this file
                accurate_path = calculate_accurate_path(ts_file)
                
                # Replace ALL overly long paths with the accurate one
                patterns_fixed = 0
                ...
                if content != original_content:
                    ...
                    print(f"✅ {relative_path} - Fixed to {accurate_path} ({patterns_fixed} patterns)")
                    files_updated += 1
    ...
    print(f"\nAll import paths fixed with accurate depth calculation!")
    print(f"📁 Files Updated: {files_updated}")
    print(f"🔄 Total Patterns Fixed: {total_patterns_fixed}")
    
    # Test critical files to verify the paths are correct
