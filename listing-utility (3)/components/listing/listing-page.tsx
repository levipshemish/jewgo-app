...
export function ListingPage({ data, className = "", loading = false, error }: ListingPageProps) {
  if (loading) {
    return (
      <div className={`w-full max-w-sm mx-auto px-4 sm:px-0 mt-4 ${className}`}>
  ...
  return (
    <div className={`w-full max-w-sm mx-auto px-4 sm:px-0 space-y-2 pb-16 mt-4 ${className}`}>
      ...
      <div className="relative">
        ...
        <div className="absolute top-0 left-0 right-0 z-10">
          <ListingHeader
            ...
            onShare={safeData.header?.onShare}
      ...
      <div className="space-y-1">
        ...
        {safeData.address && (
          <div className="rounded-2xl p-0.5 text-center">
            <span className="text-sm text-gray-900">{safeData.address}</span>
          </div>
        )}
