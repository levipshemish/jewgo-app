...
export const ShulFilters: React.FC<ShulFiltersProps> = ({
  isOpen,
  onClose,
  onApplyFilters,
  currentFilters,
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      {/* Filters Modal */}
      <div
        className="shul-filters-modal relative bg-white rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-hidden"
        style={{
      ...
      >
        ...
        <div className="max-h-[calc(90vh-140px)] overflow-y-auto p-4 space-y-6">
          ...
          <div>
            ...
            <select
              value={filters.shulType || ''}
              onChange={(e) => handleFilterChange('shulType', e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              disabled={loading}
            >
              <option value="">All Types</option>
              <option value="synagogue">Synagogue</option>
              <option value="shtiebel">Shtiebel</option>
              <option value="minyan">Minyan</option>
              <option value="community_center">Community Center</option>
              <option value="chabad_house">Chabad House</option>
          ...
          <div>
            ...
            <div className="space-y-2">
              <label className="flex items-center space-x-2">
                <input
                  type="checkbox"
                  checked={filters.hasMikvah || false}
                  onChange={(e) => handleFilterChange('hasMikvah', e.target.checked)}
                  className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                />
                <span className="text-sm text-gray-700">Mikvah Available</span>
              </label>
              <label className="flex items-center space-x-2">
                <input
                  type="checkbox"
                  checked={filters.hasKiddush || false}
                  onChange={(e) => handleFilterChange('hasKiddush', e.target.checked)}
                  className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                />
                <span className="text-sm text-gray-700">Kiddush Available</span>
              </label>
              <label className="flex items-center space-x-2">
                <input
                  type="checkbox"
                  checked={filters.hasKosherFood || false}
                  onChange={(e) => handleFilterChange('hasKosherFood', e.target.checked)}
                  className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                />
                <span className="text-sm text-gray-700">Kosher Food Available</span>
              </label>
              <label className="flex items-center space-x-2">
                <input
                  type="checkbox"
                  checked={filters.hasHebrewSchool || false}
                  onChange={(e) => handleFilterChange('hasHebrewSchool', e.target.checked)}
                  className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                />
                <span className="text-sm text-gray-700">Hebrew School</span>
              </label>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-2">
              ...
              <label className="flex items-center space-x-2">
                <input
                  type="checkbox"
                  checked={filters.hasWheelchairAccess || false}
                  onChange={(e) => handleFilterChange('hasWheelchairAccess', e.target.checked)}
                  className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                />
              ...
              <label className="flex items-center space-x-2">
                <input
                  type="checkbox"
                  checked={filters.hasSeparateSeating || false}
                  onChange={(e) => handleFilterChange('hasSeparateSeating', e.target.checked)}
                  className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
                />
                <span className="text-sm text-gray-700">Separate Seating</span>
              </label>
            </div>
          </div>
          ...
          <div>
            <label className="flex items-center space-x-2">
              <input
                type="checkbox"
                checked={filters.hasDailyMincha || false}
                onChange={(e) => handleFilterChange('hasDailyMincha', e.target.checked)}
                className="w-4 h-4 text-blue-600 border-gray-300 rounded focus:ring-blue-500"
              />
              <span className="text-sm font-medium text-gray-700">Daily Mincha/Maariv</span>
            </label>
          </div>
        </div>
