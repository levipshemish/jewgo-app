...
export default function NotificationPreferences({ 
  preferences: initialPreferences, onSave, className = '' 
        ...
        <div className="flex items-start space-x-3">
          ...
          <div>
            ...
            <p className="text-sm text-blue-700 mt-1">
              You have {safeFilter(preferences, (p: NotificationPreference) => p.email || p.push || p.sms).length} active notification types.
              {safeFilter(preferences, (p: NotificationPreference) => p.email).length > 0 && ` ${safeFilter(preferences, (p: NotificationPreference) => p.email).length} via email,`}
              {safeFilter(preferences, (p: NotificationPreference) => p.push).length > 0 && ` ${safeFilter(preferences, (p: NotificationPreference) => p.push).length} via push,`}
              {safeFilter(preferences, (p: NotificationPreference) => p.sms).length > 0 && ` ${safeFilter(preferences, (p: NotificationPreference) => p.sms).length} via SMS`}
            </p>
          </div>
        </div>
      </div>
