...
export default function EnhancedMarketplaceGrid({
  ...
  const getGridColsClass = () => {
    switch (gridCols) {
      ...
      case "4":
        return "grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5"
      case "5":
        return "grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-5"
      case "6":
        return "grid-cols-2 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-6"
      default:
        return "grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5"
    }
  }
  ...
  if (loading) {
    return (
      <div className={cn("grid gap-3", getGridColsClass(), className)}>
        {Array.from({ length: skeletonCount }).map((_, i) => (
          <EnhancedMarketplaceCardSkeleton key={i} variant={variant} />
        ))}
      </div>
    )
  }
    ...
    return (
      <div className="text-center py-12">
        <div className="max-w-md mx-auto">
          ...
          <p className="text-gray-500 mb-4">Try adjusting your search criteria or check back later for new listings.</p>
          <button
