...
export default function RestaurantDatabaseClient({
  ...
  const _fetchData = async () => {
    ...
    try {
      ...
      const _res = await fetch(`/api/admin/restaurants?${params.toString()}`, { cache: 'no-store' });
  ...
  const _onDelete = async (id: number) => {
    ...
    try {
      const _res = await fetch(`/api/admin/restaurants?id=${id}`, {
        method: 'DELETE',
        headers: {
          'x-csrf-token': csrf || '',
        },
  ...
  const _onBulkAction = async (action: string, ids: string[]) => {
    ...
    setLoading(true);
    try {
      const _res = await fetch('/api/admin/restaurants/bulk', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'x-csrf-token': csrf || '',
  ...
  const _onExport = async () => {
    try {
      ...
      const _res = await fetch(`/api/admin/restaurants/export?${params.toString()}`, {
        headers: {
          'x-csrf-token': csrf || '',
        },
      });
