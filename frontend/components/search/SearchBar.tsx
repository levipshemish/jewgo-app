...
export default function SearchBar({ onSearch, placeholder: _placeholder = "Search restaurants, agencies, or dietary preferences..." }: SearchBarProps) {
  ...
  }, [query, placesApiError, fetchPlaceSuggestions]);
  ...
  const searchSuggestions = [
    ...
    { label: 'KM Certified', value: 'KM', icon: '🥛', color: 'bg-agency-km' },
            { label: 'Kosher Miami', value: 'Kosher Miami', icon: '🍽️', color: 'bg-agency-kosher-miami' },
  ...
  const filteredSuggestions = safeFilter(searchSuggestions, (suggestion: { label: string; value: string; icon: string; color: string }) =>
